...
function loadStartListener() {
  store.dispatch('player/setAudioLoading', true);
}
function canplayListener() {
  store.dispatch('player/setAudioLoading', false);
}
function endedListener() {
  store.dispatch('player/audioEnd');
}
function timeupdateListener() {
  store.dispatch('player/audioUpdate', getTime(event));
}
function errorListener(err) {
  console.warn('Error on playing audio', err);
  store.dispatch('player/audioError', err);
}
